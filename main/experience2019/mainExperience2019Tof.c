...
static IOExpander ioExpanderArray[MAIN_EXPERIENCE_2019_IO_EXPANDER_LIST_LENGTH];

// Multiplexer
static MultiplexerList multiplexerList;
static Multiplexer multiplexerArray[MAIN_EXPERIENCE_2019_MULTIPLEXER_LIST_LENGTH];
...
void mainBoardExperience2019IOExpanderListInitDrivers(I2cBus* i2cBus) {
    // IO Expander List
    appendString(getDebugOutputStreamLogger(), "IO Expander List ...");
    initIOExpanderList(&ioExpanderList, (IOExpander(*)[]) & ioExpanderArray, MAIN_EXPERIENCE_2019_IO_EXPANDER_LIST_LENGTH);
...
MultiplexerList* mainExperience2019MultiplexerListInitDrivers(I2cBus* i2cBus) {
    appendString(getDebugOutputStreamLogger(), "Multiplexer List ...");
    initMultiplexerList(&multiplexerList, (Multiplexer(*)[]) & multiplexerArray, MAIN_EXPERIENCE_2019_MULTIPLEXER_LIST_LENGTH);
...
TofSensorList* mainExperience2019TofInitDrivers(I2cBus* i2cBus) {
    mainExperience2019MultiplexerListInitDrivers(i2cBus);

    mainBoardExperience2019IOExpanderListInitDrivers(i2cBus);

    if (MAIN_EXPERIENCE_2019_TOF_SENSOR_LIST_COUNT > 0) {
        tofSensorArray[0].name = "EXP. LEFT";
    ...
    if (MAIN_EXPERIENCE_2019_TOF_SENSOR_LIST_COUNT > 1) {
        tofSensorArray[1].name = "EXP. RIGHT";
