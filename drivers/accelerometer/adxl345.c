...
#define ADXL345_DEBUG_REG_VALUE_MG_COLUMN_LENGTH	 15
...
void adxl345_debugMainRegisterAcceleratorData(OutputStream* outputStream, unsigned char low, unsigned char high, int value, float mgValue, const char* regName) {
	...
	appendDecTableData(outputStream, value, ADXL345_DEBUG_REG_VALUE_DEC_COLUMN_LENGTH);
    appendDecfTableData(outputStream, mgValue, ADXL345_DEBUG_REG_VALUE_HEX_COLUMN_LENGTH);
	appendEndOfTableColumn(outputStream, 0);
}
...
void adxl345_debugValueRegisterList(OutputStream* outputStream, I2cBusConnection* i2cBusConnection, AccelerometerData* accelerometerData) {
    ...
    appendStringAndDec(outputStream, "SAMPLE COUNT: ", sampleCount);
...
unsigned char adx345_readDeviceID(I2cBusConnection* i2cBusConnection) {
    return adxl345_read8(i2cBusConnection, REG_DEVID);
}

unsigned char adx345_readSampleCount(I2cBusConnection* i2cBusConnection) {
    return adxl345_read8(i2cBusConnection, REG_FIFO_STATUS & FIFO_STATUS_ENTRIES_MASK);
}
...
void adxl345_setupInterruptOnSingleTapOnInt1(I2cBusConnection* i2cBusConnection, 
    ...
    unsigned char deviceId = adx345_readDeviceID(i2cBusConnection);
    ...
    adxl345_write8(i2cBusConnection, REG_THRESH_TAP, thresholdTap);
    adxl345_write8(i2cBusConnection, REG_DUR, thresholdDuration);
...
float adxl345_convertRawToMilliG(int rawValue, unsigned char dataFormat) {
    switch (dataFormat) {
...
void adxl345_readAccelerometerData(I2cBusConnection* i2cBusConnection, AccelerometerData* accelerometerData) {
    ...
    accelerometerData->milligXValue = adxl345_convertRawToMilliG(accelerometerData->xRawValue, accelerometerData->rangeMask);
    ...
    accelerometerData->milligYValue = adxl345_convertRawToMilliG(accelerometerData->yRawValue, accelerometerData->rangeMask);
    ...
    accelerometerData->milligZValue = adxl345_convertRawToMilliG(accelerometerData->zRawValue, accelerometerData->rangeMask);
}
